...
test('submit form', function () {
    ...
    expect(UserModel::all())->toHaveCount(1);
    ...
    expect(public_path('uploads/'.$avatar))->toBeFile();
});
...
test('edit form', function () {
    ...
    expect($this->crawler()->filter("select[name='tags[]'] option"))->toHaveCount(50);
    expect($this->crawler()->filter("select[name='tags[]'] option[selected]"))->toHaveCount(5);
});
test('update form', function () {
    ...
    expect('hello world')->toEqual($user->username);
});
test('update form with rule', function () {
    seedsTable(10);
